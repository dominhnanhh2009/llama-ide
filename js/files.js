(function () {
  ...
  function loadText(text, name, handle) {
    var value = IDE.Json.parse(text); IDE.state.document = value; IDE.state.disabledTools = []; IDE.state.lastResponse = null; IDE.state.responseOpen = false; IDE.state.rawText = text; IDE.state.fileName = name || "untitled.json"; IDE.state.fileHandle = handle || null; IDE.setDirty(false); IDE.App.refreshAll();
  }
